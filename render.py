...
def render_set(model_path, extra_mark, name, iteration, views, gaussians, pipeline, background, no_fine, render_test=False, reconstruct=False):
    ...
    if extra_mark == "endolgs":
        lang_path = os.path.join(model_path, name, "ours_{}".format(iteration), "lang")
        lang_npy_path = os.path.join(model_path, name, "ours_{}".format(iteration), "lang_npy")
        makedirs(lang_path, exist_ok=True)
        makedirs(lang_npy_path, exist_ok=True)
        lang_features = []

    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        stage = 'coarse' if no_fine else 'fine-lang'
        ...
        if extra_mark == "endolgs":
            lang_features.append(rendering["language_feature_image"].cpu())
    ...
    count = 0
    print("writing language features.")
    if extra_mark == "endolgs" and len(lang_features) != 0 and lang_path != None:
        for image in tqdm(lang_features):
            image = image.float()
            np.save(os.path.join(lang_npy_path, '{0:05d}'.format(count) + ".npy"), image.permute(1, 2, 0).cpu().numpy())
            torchvision.utils.save_image(image, os.path.join(lang_path, '{0:05d}'.format(count) + ".png"))
            count += 1
    ...
    lang_arry = torch.stack(lang_features, dim=0).permute(0, 2, 3, 1)
    lang_arry = (lang_arry * 255).clip(0, 255).cpu().numpy().astype(np.uint8)
    ratio = 0.5
    blend_arry = lang_arry * ratio + render_array * (1 - ratio)
    blend_arry = blend_arry.astype(np.uint8)
    imageio.mimwrite(os.path.join(model_path, name, "ours_{}".format(iteration), 'feature_video.mp4'), lang_arry, fps=30, quality=8)
    imageio.mimwrite(os.path.join(model_path, name, "ours_{}".format(iteration), 'blend_video.mp4'), blend_arry, fps=30, quality=8)
    ...
    if reconstruct:
        reconstruct_point_cloud(render_images, render_depths, camera_parameters, name)
...
def render_sets(dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video: bool, reconstruct: bool):
    ...
    with torch.no_grad():
        ...
        if not skip_train:
            render_set(dataset.model_path, dataset.extra_mark, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, dataset.no_fine, reconstruct=False)
        if not skip_test:
            render_set(dataset.model_path, dataset.extra_mark, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, dataset.no_fine, reconstruct=reconstruct)
        if not skip_video:
            render_set(dataset.model_path, dataset.extra_mark,"video", scene.loaded_iter, scene.getVideoCameras(), gaussians, pipeline, background, dataset.no_fine, render_test=True, reconstruct=False)

def reconstruct_point_cloud(images, depths, camera_parameters, name):
    import cv2
    import copy
