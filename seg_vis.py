...
def process_frames(unique_ids, num_frames, mask_dir, image_dir, output_dir, begin_idx=1, end_str="png"):
    ...
    for i in tqdm(range(begin_idx, num_frames + begin_idx)):
        image_path = f'{image_dir}/frame-{(i-1):06}.color.png'
        mask_path = f'{mask_dir}/frame-{(i-1):06}.color.npy'
            ...
            if boundary.size > 0:
                ...
                for y in range(min_y, max_y + 1):
                    for x in range(min_x, max_x + 1):
                        # print(mask.shape)
                        if mask_for_id[y, x] and (
                                x == 0 or y == 0 or x == mask.shape[1] - 1 or y == mask.shape[0] - 1 or not mask_for_id[
                            y - 1, x] or not mask_for_id[y + 1, x] or
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default='./segmentation')
