...
from utils.loss_utils import l1_loss, ssim, lpips_loss, TV_loss, cos_loss
...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    first_iter = 0
    gaussians.training_setup(opt, init_from_stage=stage)
    if checkpoint:
    ...
    for iteration in range(first_iter, final_iter+1):
        ...
        for viewpoint_cam in viewpoint_cams:
            ...
            if dataset.extra_mark == 'endolgs':
                gt_language_feature = viewpoint_cam.original_language_feature.cuda().float()
                language_feature_mask = viewpoint_cam.language_feature_mask.cuda().float()
                gt_language_features.append(gt_language_feature.unsqueeze(0))
                language_feature_masks.append(language_feature_mask.unsqueeze(0))
            ...
            language_features.append(language_feature.unsqueeze(0))
        ...
        language_features = torch.cat(language_features, 0)
        if dataset.extra_mark == 'endolgs':
            gt_language_features = torch.cat(gt_language_features, 0)
            language_feature_masks = torch.cat(language_feature_masks, 0)

        # color loss
        # no mask
        # Ll1 = l1_loss(rendered_images, gt_images)
        ...
        if dataset.extra_mark == 'endolgs' and stage == 'fine-lang':
            # no mask
            loss += args.lam * l1_loss(language_features * language_feature_masks, gt_language_features * language_feature_masks)
            loss += args.beta * cos_loss(language_features * language_feature_masks, gt_language_features * language_feature_masks)

            # use mask
            # loss += args.lam * l1_loss(language_features * language_feature_masks, gt_language_features * language_feature_masks, masks)
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter :
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0 :
                    ...
                    gaussians.densify(densify_threshold, opacity_threshold, scene.cameras_extent, size_threshold, stage)
                    
                if iteration > opt.pruning_from_iter and iteration % opt.pruning_interval == 0:
                    size_threshold = 40 if iteration > opt.opacity_reset_interval else None
                    gaussians.prune(densify_threshold, opacity_threshold, scene.cameras_extent, size_threshold, stage)
            ...
            if (iteration in checkpoint_iterations):
                ...
                torch.save((gaussians.capture(opt.include_featur), iteration), scene.model_path + "/chkpnt" + str(iteration) + ".pth")
...
def training(dataset, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, expname, args):
    ...
    if not args.no_fine:
        if dataset.extra_mark == 'endolgs':
            scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations,
                                checkpoint_iterations, checkpoint, debug_from,
                                gaussians, scene, "fine-lang", tb_writer, opt.iterations, timer, args)
        else:
            scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations,
                                 checkpoint_iterations, checkpoint, debug_from,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[2000, 3000])
    ...
    training(lp.extract(args), hp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, \
        args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.expname, args)
