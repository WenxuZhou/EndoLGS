...
class Camera_EndoLGS(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, depth, mask, gt_alpha_mask,
                 point_feature, seg_mask, image_name, uid,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0,
                 data_device="cuda", time=0, Znear=None, Zfar=None
                 ):
        super(Camera_EndoLGS, self).__init__()
        ...
        self.original_image = image.clamp(0.0, 1.0)
        self.original_depth = depth
        self.original_language_feature = point_feature
        self.language_feature_mask = seg_mask
        self.image_width = self.original_image.shape[2]
        self.image_height = self.original_image.shape[1]
        if gt_alpha_mask is not None:
            self.original_image *= gt_alpha_mask
