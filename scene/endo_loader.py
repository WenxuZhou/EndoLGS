...
from scene.cameras import Camera, Camera_EndoLGS
...
class EndoNeRF_Dataset(object):
    def __init__(
        ...
        self.train_idxs = [i for i in range(n_frames)]
...
class EndoLangSplat_Dataset(object):
    def __init__(
            self,
            datadir,
            downsample=1.0,
    ...
    def load_meta(self):
        ...
        # get paths of images, depths, masks, etc.
        agg_fn = lambda filetype: sorted(glob.glob(os.path.join(self.root_dir, filetype, "*.png")))
        agg_lang_s = lambda filetype: sorted(glob.glob(os.path.join(self.root_dir, filetype, "*_s.npy")))
        agg_lang_f = lambda filetype: sorted(glob.glob(os.path.join(self.root_dir, filetype, "*_f.npy")))
        ...
        if self.mode == 'binocular':
            ...
            raise ValueError(f"{self.mode} has not been implemented.")
        self.masks_paths = agg_fn("masks")
        self.lang_s_path = agg_lang_s("clip_features_dim3")
        self.lang_f_path = agg_lang_f("clip_features_dim3")

        assert len(self.image_paths) == poses.shape[0], "the number of images should equal to the number of poses"
        assert len(self.depth_paths) == poses.shape[0], "the number of depth images should equal to number of poses"
        assert len(self.masks_paths) == poses.shape[0], "the number of masks should equal to the number of poses"
        assert len(self.lang_s_path) == poses.shape[0], "the number of feature_s should equal to the number of poses"
        assert len(self.lang_f_path) == poses.shape[0], "the number of feature_f should equal to the number of poses"

    def get_language_feature(self, feature_level, lang_s_path, lang_f_path, image_height, image_width):
        seg_map = torch.from_numpy(np.load(lang_s_path))
        feature_map = torch.from_numpy(np.load(lang_f_path))
    ...
    def format_infos(self, split):
        ...
        for idx in tqdm(idxs):
            ...
            lang_s_path, lang_f_path = self.lang_s_path[idx], self.lang_f_path[idx]
            point_feature, seg_mask = self.get_language_feature(self.feature_level, lang_s_path, lang_f_path, image_height, image_width)
            cameras.append(Camera_EndoLGS(colmap_id=idx, R=R, T=T, FoVx=FovX, FoVy=FovY, image=image, depth=depth, mask=mask,
                                          point_feature=point_feature, seg_mask=seg_mask, gt_alpha_mask=None, image_name=f"{idx}",
                                          uid=idx, data_device=torch.device("cuda"), time=time, Znear=None, Zfar=None))
        return cameras
    ...
    def search_pts_colors_with_motion(self, ref_pts, ref_color, ref_mask, ref_c2w):
        ...
        for j in range(1, len(self.image_poses), interval):
            ...
            out_vis_mask = ((X_Z > (self.img_wh[0] - 1)) + (X_Z < 0) + (Y_Z > (self.img_wh[1] - 1)) + (Y_Z < 0)) > 0
            ...
            index_mask = compl_mask[mask_refine]
...
class Hamlyn_Dataset(object):
    def __init__(
        ...
        intrinsics = Intrinsics(
            width=self.img_wh[0], height=self.img_wh[1], focal_x=intrinsics_matrix[0, 0],
            focal_y=intrinsics_matrix[1, 1],
        ...
        assert len(imgs) == len(masks) == len(
            depths), "the number of images should equal to the number of masks and depths"
    ...
    def get_init_pts(self):
        if self.mode == 'binocular':
            ...
            color, depth, mask = self.imgs[initial_idx].numpy(), self.depths[initial_idx].numpy(), self.masks[
                initial_idx].numpy()
