...
class GaussianModel:
    ...
    def training_setup(self, training_args, joint_train=False, no_dlang=False, init_from_stage=Literal['coarse', 'fine', 'fine-lang']):
        self.percent_dense = training_args.percent_dense
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self._deformation_accum = torch.zeros((self.get_xyz.shape[0],3),device="cuda")

        if init_from_stage == "fine-lang":
            # if self._language_feature.shape[-1] == 3:
            ...
            if self._language_feature is not None:
                l.append(
                    {'params': [self._language_feature], 'lr': training_args.language_feature_lr,
            ...
            if stored_state is not None:
                ...
                if group["name"] == 'language_feature':
                    ...
                    group["params"][0] = nn.Parameter(
                        torch.cat((group["params"][0], extension_tensor), dim=0).requires_grad_(train_language_feature))
                else:
                    group["params"][0] = nn.Parameter(
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, stage, N=2):
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, stage):
        # Extract points that satisfy the gradient condition
    ...
    def densify(self, max_grad, min_opacity, extent, max_screen_size, stage):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        self.densify_and_clone(grads, max_grad, extent, stage)
        self.densify_and_split(grads, max_grad, extent, stage)
    ...
    def standard_constaint(self):
        ...
        lang = self._language_feature.datach()
        time =  torch.tensor(0).to("cuda").repeat(means3D.shape[0],1)
        means3D_deform, scales_deform, rotations_deform, lang_deform, coff = self._deformation(means3D, scales, rotations, opacity, time, lang)
    ...
    def generate_multi_feature_centers(self, sample_num=100, init_from_stage=Literal['coarse', 'fine', 'fine-lang']):
        '''
        ...
        if init_from_stage == 'fine':
            ...
            multi_language_feature = torch.zeros(language_feature.shape[0], centers_num,
                                                 language_feature.shape[-1]).cuda()
            ...
            for _ in range(sample_num):
                ...
                language_feature_precomp_final.append(
                    self._deformation(self.get_xyz, self.get_scaling, self.get_rotation, self.get_opacity,
                                      random_time, language_feature_precomp)[-2])
