...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], load_coarse=False):
        ...
        elif os.path.exists(os.path.join(args.source_path, "labels")) and args.extra_mark == 'endolgs':
            scene_info = sceneLoadTypeCallbacks["endolgs"](args.source_path, mode=args.mode)
            print("Found poses_bounds.py and extra marks with EndoLGS")
        ...
        if self.loaded_iter:
            ...
            if args.extra_mark == 'endonerf' or args.extra_mark == 'endolgs':
