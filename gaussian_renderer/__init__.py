...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, stage="fine"):
    ...
    coff = None
    if stage == "coarse":
        means3D_deform, scales_deform, rotations_deform, opacity_deform, language_feature_precomp_deform = means3D, scales, rotations, opacity, language_feature_precomp
    else:
        means3D_deform, scales_deform, rotations_deform, opacity_deform, language_feature_precomp_deform, coff = pc._deformation(means3D[deformation_point], scales[deformation_point],
                                                                         rotations[deformation_point], opacity[deformation_point],
                                                                         time[deformation_point], language_feature_precomp[deformation_point])
    # print(time.max())
    with torch.no_grad():
    ...
    language_feature_precomp_final = torch.zeros_like(language_feature_precomp)
    means3D_final[deformation_point] =  means3D_deform
    rotations_final[deformation_point] =  rotations_deform
    scales_final[deformation_point] =  scales_deform
    opacity_final[deformation_point] = opacity_deform
    language_feature_precomp_final[deformation_point] = language_feature_precomp_deform
    ...
    language_feature_precomp_final[~deformation_point] = language_feature_precomp[~deformation_point]
